...
def train_and_predict(model_num, path, file_extension, iteration):
    # Train the model
...
def train_classification_pointnet_model(model_num, data_folder, file_extension):
    if not model_num or not data_folder or not file_extension:
        print("Usage: python pointnet_classification_and_prediction.py --model-num <MODEL-NUM> --data-folder <DATA-FOLDER> --file-extension <FILE-EXTENSION>")
        exit(1)

    print(f"Model: {model_num} \t\tData Folder: {data_folder} \t\tFile Extension: {file_extension}")

    # Loop through the 10 iterations (1 to 10)
    for iteration in range(1, 11):
        print(f"\nStarting iteration {iteration}...")
        data_path = f"{data_folder}"
        
        # Train the model and make predictions for this iteration
        train_and_predict(model_num, data_path, file_extension, iteration)
