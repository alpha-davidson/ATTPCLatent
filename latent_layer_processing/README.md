...
uses the full frozen embedding vector with input batch normalization by default.

Add `--no-batch-norm` to probe raw embeddings. PCA, UMAP, and t-SNE are not used
before the default probe.
